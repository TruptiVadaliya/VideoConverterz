...
export default function Home() {
  ...
  const onDragEnd = (result) => {
    ...
    setImages((imgs) => reorder(imgs, result.source.index, result.destination.index));
  };
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text() || "Failed to generate video");
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col items-center py-8 px-2">
      ...
      {images.length > 0 && (
        <div className="mb-4 flex items-center gap-2">
          <label className="text-sm font-semibold">Set all durations (sec):</label>
          <input
            type="number"
            min="0.1"
            step="0.1"
      ...
      <div
        ...
        className={`w-full max-w-xl border-2 border-dashed rounded-lg p-6 mb-6 cursor-pointer transition-colors ${
          isDragActive ? "border-blue-500 bg-blue-50" : "border-gray-300 bg-white dark:bg-gray-800"
        }`}
      >
        <input {...getInputProps()} />
                    ...
                    <Draggable draggableId={img.id} index={idx}>
                      {(provided) => (
                        <div
                          ...
                          tabIndex={0}
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === " ") removeImage(img.id);
                          }}
                        >
                          <img
                            src={img.preview}
      ...
      {videoUrl && (
        <div className="w-full max-w-xl flex flex-col items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-lg shadow mt-6">
          <video src={videoUrl} controls muted className="w-full rounded" style={{ maxHeight: 400 }} />
          <a
            href={videoUrl}
            download="output.mp4"
